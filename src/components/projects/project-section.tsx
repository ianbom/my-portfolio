...
export function ProjectSection({ section }: { section: ProjectSectionData }) {
  return (
    <section className="grid gap-5 border-t border-white/10 py-10 md:grid-cols-[220px_1fr] md:py-14">
      ...
      <div className="max-w-3xl text-sm leading-7 text-[#b0b0b0] sm:text-base">
        {section.description && <p>{section.description}</p>}
        {section.items?.length && (
          <ul className={section.description ? "mt-6 space-y-3" : "space-y-3"}>
            {section.items.map((item) => (
              <li className="flex gap-3" key={item}>
                <span className="mt-3 size-1 shrink-0 rounded-full bg-[#8db7dc]" />
                <span>{item}</span>
              </li>
            ))}
          </ul>
